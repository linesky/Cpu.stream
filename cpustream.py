...
def generate():
    global mensagem
    while True:
        mensagem = subprocess.check_output("sudo sensors", shell=True).decode().strip().replace("\n","<br>").replace("\r","<br>")
...
datas = subprocess.check_output("sudo service kmod start", shell=True).decode().strip()
print(datas)   
